...
export const AgreementSection = ({ register, errors }: AgreementSectionProps) => {
  return (
    <div className="space-y-2 mb-[36px]">
      <div>
        <label className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <input
              ...
              className="w-4 h-4"
            />
            <BodySmall className="text-neutral-900">
              청소 서비스 중개 및 결제 약관 (필수)
            </BodySmall>
          </div>
        ...
        {errors.serviceAgreement && (
          <Caption className="text-red-500 mt-1">{errors.serviceAgreement.message}</Caption>
        )}
      </div>
      <div>
        <label className="flex items-center justify-between">
        ...
        {errors.entryAgreement && (
          <Caption className="text-red-500 mt-1">{errors.entryAgreement.message}</Caption>
        )}
      </div>
    </div>
  );
