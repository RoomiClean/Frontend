...
import { DisplaySmall, TitleSmall, BodyDefault } from '../atoms/Typography';
...
export const Calendar = ({
  ...
  return (
    <div>
      <div className="w-full h-full bg-neutral-100 border border-neutral-300 rounded-[20px] shadow-[0_6px_15px_0_rgba(0,0,0,0.2)] p-6">
        ...
        <div className="grid grid-cols-7 mb-6">
          {dates.map((date, index) => {
            ...
            return (
              <div
                key={`date-${date}`}
                className="aspect-square p-1 flex items-center justify-center relative"
              >
                {/* 숙소 예약 일정 */}
                {markers.isAccommodation && (
                  <div
                    className={`absolute inset-x-0 h-8 top-1/2 -translate-y-1/2 bg-primary-alpha30 ${
                      markers.accommodationPosition === 'single'
                        ? 'rounded-full mx-1 w-8 left-1/2 -translate-x-1/2 border border-primary-alpha50'
                        : markers.accommodationPosition === 'start'
                          ? 'rounded-l-full ml-1 border-l border-t border-b border-primary-alpha50'
                          : markers.accommodationPosition === 'end'
                            ? 'rounded-r-full mr-1 border-r border-t border-b border-primary-alpha50'
                            : 'border-t border-b border-primary-alpha50'
                    }`}
                  />
                )}
                ...
                {markers.isLastCleaning && !markers.isAccommodation && (
                  <div className="absolute w-8 h-8 bg-green-alpha30 border border-green-alpha50 rounded-full" />
                )}

                {/* 예약된 청소 일정 마커 */}
                {markers.isReservedCleaning && !markers.isAccommodation && (
                  <div className="absolute w-8 h-8 bg-red-alpha30 border border-red-alpha50 rounded-full" />
                )}

                {/* 오늘 날짜 테두리 */}
                {markers.isToday && (
                  <div className="absolute w-8 h-8 border-[1px] border-red-100 rounded-full" />
                )}
