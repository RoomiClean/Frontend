...
export const Calendar = ({
  ...
  return (
    <div className="w-full h-full bg-neutral-100 border border-neutral-300 rounded-[20px] shadow-[0_6px_15px_0_rgba(0,0,0,0.1)] p-6 flex flex-col">
          ...
          return (
            <div key={`date-${date}`} className="p-1 flex items-center justify-center relative">
              ...
              {markers.isLastCleaning && !markers.isAccommodation && (
                <div className="absolute w-[2.6em] h-[2.6em] bg-green-alpha30 border border-green-alpha50 rounded-full" />
              )}

              {/* 예약된 청소 일정 마커 */}
              {markers.isReservedCleaning && !markers.isAccommodation && (
                <div className="absolute w-[2.6em] h-[2.6em] bg-red-alpha30 border border-red-alpha50 rounded-full" />
              )}

              {/* 오늘 날짜 테두리 */}
              {markers.isToday && (
                <div className="absolute w-[2.6em] h-[2.6em] border-[1px] border-red-100 rounded-full" />
              )}
